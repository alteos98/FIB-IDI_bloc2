...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
	...
	case Qt::Key_O: { // perspectiva <-> ortogonal
      tipusCamera = !tipusCamera;
      break;
    }
...
void MyGLWidget::projectTransform() {
	...
	if (!tipusCamera) {
		...
		Proj = glm::ortho(-radiEsfera, radiEsfera, -radiEsfera, radiEsfera, zN, zF);
	}
	glUniformMatrix4fv(projLoc, 1, GL_FALSE, &Proj[0][0]);
}

void MyGLWidget::viewTransform() {
	glm::mat4 View = glm::translate(glm::mat4(1.0f), glm::vec3(0,0,-(radiEsfera*2)));
