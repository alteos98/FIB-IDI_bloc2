...
void MyGLWidget::modelTransformPatricio() 
{
  ...
  transform_Patricio = glm::translate(transform_Patricio, -centre_esfera);
...
void MyGLWidget::viewTransform() {
  ...
  /*OBS = centre_esfera + d * glm::vec3 (0,0,1);
  VRP = centre_esfera;
  UP = glm::vec3(0,1,0);*/

  OBS = VRP + d*glm::vec3 (0,1,1);
  VRP = glm::vec3 (0,0,0);
  UP = glm::vec3(0,1,0);
...
void MyGLWidget::calculRadiCentre() {
  centre_esfera = glm::vec3 ((Pmin.x+Pmax.x)/2,(Pmin.y+Pmax.y)/2,(Pmin.z+Pmax.z)/2);
  radi_esfera = sqrt((Pmax.x - Pmin.x)*(Pmax.x - Pmin.x) + (Pmax.y - Pmin.y)*(Pmax.y - Pmin.y) + (Pmax.z - Pmin.z)*(Pmax.z - Pmin.z)) / 2;
  d = 2 * radi_esfera;
}

void MyGLWidget::calculCaixa() {
  Pmin = glm::vec3(m.vertices()[0], m.vertices()[1], m.vertices()[2]);
  Pmax = glm::vec3(m.vertices()[0], m.vertices()[1], m.vertices()[2]);

  for (unsigned int i = 0; i < m.vertices().size(); i+=3) {
	  if (m.vertices()[i] < Pmin.x) Pmin.x = m.vertices()[i];
    if (m.vertices()[i] > Pmax.x) Pmax.x = m.vertices()[i];
    if (m.vertices()[i+1] < Pmin.y) Pmin.y = m.vertices()[i+1];
    if (m.vertices()[i+1] > Pmax.y) Pmax.y = m.vertices()[i+1];
    if (m.vertices()[i+2] < Pmin.z) Pmin.z = m.vertices()[i+2];
    if (m.vertices()[i+2] > Pmax.z) Pmax.z = m.vertices()[i+2];
  }

  /*Dos punts calculats a mà*/
  /*glm::vec3 Pmin = glm::vec3(-1,-1,-1);
  glm::vec3 Pmax = glm::vec3(1,1,1);*/

  alturaModel = Pmax.y - Pmin.y;
